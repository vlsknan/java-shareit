...
public class BookingController {
	...
	public ResponseEntity<Object> getBookings(@RequestHeader(X_SHARER_USER_ID) long userId,
		...
		BookingState state = BookingState.from(stateParam)
				.orElseThrow(() -> new MessageFailedException("Unknown state: " + stateParam));
	...
	public ResponseEntity<Object> confirmation(@RequestHeader(X_SHARER_USER_ID) int userId,
	...
	@GetMapping("/owner")
	public ResponseEntity<Object> getAllByOwnerId(@RequestHeader(X_SHARER_USER_ID) int ownerId,
											@RequestParam(defaultValue = "ALL") String state,
											@PositiveOrZero @RequestParam(defaultValue = "0") int from,
											@Positive @RequestParam(defaultValue = "10") int size) {
		BookingState bookingState = BookingState.from(state)
				.orElseThrow(() -> new MessageFailedException("Unknown state: " + state));
